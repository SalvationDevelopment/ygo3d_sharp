...
namespace YGOSharp.OCGWrapper
{
    public class Duel
    {
        ...
        public int Process()
        {
            ...
            {
                Api.process(_duelPtr);
                paused = Api.query_pause(_duelPtr);
                if (paused == 1)
                {
                    break;
                }
            ...
            try
            {
                int length = Api.get_hashed_buffer(_duelPtr, _buffer);
                byte[] arr = new byte[length];
                Marshal.Copy(_buffer, arr, 0, length);
                result = _analyzer.Invoke((GameMessage)0, null,arr);
            }
            catch (Exception)
            {
                result = -1;
